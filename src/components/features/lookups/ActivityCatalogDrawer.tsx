    ...
    >
      <FormSection title="Activity" subtitle="Which discipline performs the work. Standard across every project.">
        <FormField label="Name" htmlFor="ac-name" required error={errors.name}>
          <Input id="ac-name" value={name} error={!!errors.name} disabled={readOnly}
            onChange={(e) => { setName(e.target.value); setErrors((p) => ({ ...p, name: '' })) }} />
        </FormField>
        <FormField label="Description" htmlFor="ac-desc">
          <Textarea id="ac-desc" rows={2} value={description} disabled={readOnly} onChange={(e) => setDescription(e.target.value)} />
        </FormField>
        <FormField label="Task required" htmlFor="ac-taskreq"
          help="On: Time Entry insists on a task. Off: the Task field is hidden rather than left empty.">
          <Checkbox id="ac-taskreq" checked={taskRequired} disabled={readOnly}
        ...
        <FormField label="Default" htmlFor="ac-default" help="Offered at the top of the activity picker.">
          <Checkbox id="ac-default" checked={isDefault} disabled={readOnly}
        ...
        <FormField label="Non-project time" htmlFor="ac-nonproject"
          help="Holiday, sick leave, training. Logged like any hours, but never budgeted and never assigned to a work package.">
          <Checkbox id="ac-nonproject" checked={nonProject} disabled={readOnly}
        ...
        <FormField label="Active" htmlFor="ac-active"
          help={inUse > 0
            ? `Used by ${inUse} record${inUse === 1 ? '' : 's'}. Deactivating hides it from the pickers; those records keep it.`
            : 'Inactive activities stay on old records but disappear from the pickers.'}>
          <Checkbox id="ac-active" checked={active} disabled={readOnly}
      ...
      {!nonProject && (
        <FormSection title="Tasks" subtitle="The tasks Time Entry offers once this activity is chosen.">
          ...
          <FormField label="Linked tasks" htmlFor="ac-tasks"
            help="A task can belong to several activities — “Conceptual Design” sits under both design disciplines.">
            <MultiSelect id="ac-tasks" value={taskIds} options={taskOptions} disabled={readOnly}
