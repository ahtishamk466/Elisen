...
| GcpCertBasesPage | populated / no-results / empty, one flat table | `features/gcp/GcpCertBasesPage.tsx` | **Cert Basis** (not "Cert Bases" — client instruction, 2026-09-24: use this exact spelling everywhere the term appears, nav label included) at `/gcp/cert-bases`: **one table**, every basis's regulations one after another — client instruction, 2026-09-24, replacing a master–detail (rail + a separate selected-basis pane) that read as two sections instead of one screen. Each basis is a group row (`colSpan` name + TCDS/"Project-specific" + regulation count, own `ActionsMenu` Edit/Delete — the only place those two actions live now) followed by its own Regulation Section, Regulation Amdt, Regulation Title, Status, Actions rows (View/Edit/Remove via `RegulationDetailDrawer`/`RegulationDrawer`, reused unchanged from the Regulations library). One shared sort state (the frozen header's `SortableTh`s) reorders *within* each basis's own rows, not across the whole table: every visible regulation sorts together via one `useTableSort` call (hooks can't run in a per-basis loop), then each group filters that sorted list back down to its own `basisId`, which preserves the group's relative order for free. Header carries search (still narrows to matching bases, each showing all its own regulations, same as before), a secondary **Import** button (`Download` icon — client instruction, 2026-09-24: was `Upload`, backwards for an action that brings data *into* the app, opens `CertBasisImportDrawer`) and the primary **Add Cert Basis** (opens `CertBasisDrawer`). Linking a basis to a project stays GCP Projects' own Certification Basis step, not this page — this is where a basis's own identity and rule set live. `useGcpFlowStore`'s `bases` seeds the two `CertBasis` records `projectBasis` already pointed at (`seed-basis-tp-60`/`-tp-64`) — it was `[]`, a dangling reference that left this page always on its empty state and `FlowStepBasis` reading no existing regulations for either seeded project |
