...
| Avatar | sm (20px) / md (36px) | `patterns/Avatar.tsx` | Initials only, **one fill and one text colour, no tone prop** — colour-coding people by role rendered the same person differently on different screens |
...
| HoursByPersonTab | ready / loading / empty | `features/timesheet/HoursByPersonTab.tsx` | Everyone's hours aggregated per person, expandable. Same visual language as WorkPackageCard and the project Team tab: chevron, name (**designation under it**), status badge, then figures with the meter and percentage last. Overtime / Banked / Non-project are separate columns — no budget covers them, so they are never inside Actual. Badge = overall position; the `N of M over` chip = what a blended average hides |
| PersonHoursDetail | with / without non-project time | `features/timesheet/PersonHoursDetail.tsx` | One person's activities, **grouped flat by project with a sub-total per project** — cross-project detail is three levels deep and a three-deep accordion is unreadable. Opens the 5 busiest projects with the rest behind a button that states the count. Non-project time is a final group with no budget columns |
