...
| CodeWithSubname | code alone / code + name | `features/gcp/CodeWithSubname.tsx` | A reference code with what it's assigned to underneath — code on top, name below, capped to **8 characters** + `…` (the client's own number), full name **and** the code, on native `title` hover — sat on the name's span alone before, so hovering the code itself (half the cell) showed nothing (client instruction, 2026-09-25: "user hover on any [part of the] column... show tooltip of info of code with name"), moved to the wrapping span, both lines. Client instruction, 2026-09-25: a bare Discipline/MOC/FOC code (`AP-01`) told the reader nothing about who or what it meant — "user ko asani ho ke code kis ke against assign hai" — using the exact example "AP-01 code ke against ye naam hai [specialist name]". Character-capped, not `Truncate`'s line-clamp — the client asked for a fixed letter count, not however much fits the column. Used in `FlowStepProject` (Discipline/MOC/FOC columns, looked up from `useGcpStore`'s `disciplines`/`mocs`/`focs` by code — `elisenDiscipline`/`title`/`authoritySpecialist` respectively) and `FlowStepPlan`'s GCP Data table (same three lookups, same columns). Not applied to the reference tables that already show code *and* name as two full columns side by side (`GcpFocTab`, `GcpDisciplineTab`) — those already answer the same question without truncation |
