...
| Component | Variants | Location | Purpose |
|-----------|----------|----------|---------|
| GcpFlowPage | 5 steps, each gated on the one before | `features/gcp/flow/GcpFlowPage.tsx` | The client's requested alternative to the tab menu: one route at `/gcp/flow` that runs Project → Certification Basis → Initialize → Certification Plan → Dashboard, `Stepper` on top, `?step=` and `?project=` in the URL. A step opens only once the step before it has produced what it needs |
| FlowStepProject | populated / no-results / loading / error | `features/gcp/flow/FlowStepProject.tsx` | Step 1 — a sortable table of TCCA projects (TCCA Project #, Project Description, Elisen Project, Applicable, Affected, GCP Progress, Opened, Action), matching the standalone TCCA Projects list's columns and table style rather than cards. "Start GCP" / "Continue GCP" resumes a project at the furthest step its data supports |
| FlowStepBasis | new / existing basis, empty rule list | `features/gcp/flow/FlowStepBasis.tsx` | Step 2 — the merge the client asked for: aircraft model and type certificate (legacy Master Cert Basis) plus the regulations allocated to it with their amendment, title and codes (legacy Cert Basis), linked to the project here instead of on a separate Associate screen |
| FlowStepInitialize | reviewed count, affected count | `features/gcp/flow/FlowStepInitialize.tsx` | Step 3 — mark each rule in the basis affected or not; confirming writes the plan |
| FlowStepPlan | planned / not planned per rule | `features/gcp/flow/FlowStepPlan.tsx` | Step 4 — the affected rules, and which of them still have nothing planned |
| FlowStepDashboard | rule list + editor, empty | `features/gcp/flow/FlowStepDashboard.tsx` | Step 5 — per rule: requirement text, a Complete checkbox, method (seeded from Default Moc Text), comments, and the compliance items |
| GcpItemDrawer | — | `features/gcp/flow/GcpItemDrawer.tsx` | One discipline's plan for one rule: discipline, MOC, finding by, document. Free-text until the client's lists arrive |
...
| GcpPeoplePage | tabs: FOC / Delegation / Discipline | `features/gcp/GcpPeoplePage.tsx` | Who may find or recommend compliance, at `/gcp/people` |
